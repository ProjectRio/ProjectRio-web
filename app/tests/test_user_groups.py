...
def test_patron_refresh():

    un_length = random.randint(3,20)
    pw_length = 20

    # Need to mimic patron emails
    user_dict = {'Username': ''.join(random.choices(string.ascii_letters, k=un_length)), 
                 'Email': 'nathanielburkeperkins@gmail.com',
                 'Password': ''.join(random.choices(string.ascii_letters + string.digits + string.punctuation , k=pw_length))}
    sponsor = User(user_dict)
    sponsor.register()
    sponsor.verify_user()
    ...
    admin.register()
    admin.verify_user()
    assert admin.success == True

    assert admin.add_to_group('admin') == True

    admin.register_api_key()
    
    response = requests.get(f'http://127.0.0.1:5000/patreon/refresh/?api_key={admin.ak}')

    # Check that sponsor is in the patron group
    sponsor.refresh()
    assert 'Patron: Rookie' in sponsor.groups

    # Check that sponsor is only sponsor of 1 community (originally 2 but removed because of patron limits)
    communityA.refresh()
    communityB.refresh()

    assert (communityA.sponsor == None or communityB.sponsor == None )
